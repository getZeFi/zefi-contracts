...
const Compound = require("../build/Compound");
...
describe("Invest Manager with Compound", function () {
    ...
    describe("Investment", () => {
        ...
        async function removeInvestment(tokenAddress, fraction) {

            let tx, txReceipt;
            let investInEth = (tokenAddress == ETH_TOKEN) ? true : false;

            await addInvestment(tokenAddress, parseEther('0.1'), 365);
        ...
        describe("Add Investment", () => {
            ...
            it('should invest in ERC20 for 1 year and gain interests', async () => {
                await addInvestment(token.contractAddress, parseEther('1'), 365);
            });

            it('should invest in ETH for 1 year and gain interests', async () => {
                await addInvestment(ETH_TOKEN, parseEther('1'), 365);
            });
        });
        ...
        describe("Remove Investment", () => {
            function testRemoveERC20Investment(fraction) {
                it(`should remove ${fraction / 100}% of an ERC20 investment`, async () => {
                    await removeInvestment(token.contractAddress, fraction);
                });
            }
